...
import { KeysRdfResolveQuadPattern } from '@prov4itdata/context-entries';
...
export class FederatedQuadSource implements IQuadSource {
  ...
  public match(subject: RDF.Term, predicate: RDF.Term, object: RDF.Term, graph: RDF.Term): AsyncIterator<RDF.Quad> {
    ...
    const metadata: Record<string, any> = { totalItems: 0, __prov: new Array<Record<string, any>>() };
    ...
    const proxyIt: Promise<AsyncIterator<RDF.Quad>[]> = Promise.all(this.sources.map(async source => {
      ...
      // Handle the metadata & prov from this source
      output.data.getProperty('metadata', (subMetadata: Record<string, any>) => {
        if (subMetadata.__prov) {
          const prov = subMetadata.totalItems ? { ...subMetadata.__prov, totalItems: subMetadata.totalItems } : subMetadata.__prov;
          metadata.__prov.push(prov);
        }
